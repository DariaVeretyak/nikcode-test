(() => {
    ...
    const customers_namespaceObject = JSON.parse('[{"name":"Jane Cooper","email":"jane@microsoft.com","added":"2024-01-15","role":"User"},{"name":"Floyd Miles","email":"floyd@yahoo.com","added":"2024-03-21","role":"User"},{"name":"Ronald Richards","email":"ronald@adobe.com","added":"2023-01-07","role":"Admin"},{"name":"Marvin McKinney","email":"marvin@tesla.com","added":"2024-02-29","role":"User"},{"name":"Jerome Bell","email":"jerome@google.com","added":"2024-02-05","role":"User"},{"name":"Kathryn Murphy","email":"kathryn@microsoft.com","added":"2024-09-30","role":"User"},{"name":"Jacob Jones","email":"jacob@yahoo.com","added":"2022-07-15","role":"User"},{"name":"Kristin Watson","email":"kristin@facebook.com","added":"2024-06-01","role":"User"}]');
    const customersRows = document.querySelector(".customers-rows");
    function addCustomers(arr, place) {
        arr.forEach((customer => {
            place.insertAdjacentHTML("beforeend", `\n        <tr class="user">\n          <td class="text-left">${customer.name}</td>\n          <td class="text-left nowrap"><a href="mailto:${customer.email}">${customer.email}</a></td>\n          <td class="text-left">${customer.added}</td>\n          <td class="text-left">${customer.role}</td>\n          <td>\n            <div class="td-actions">\n              <button type="button" class="btn-remove">\n                <svg class="btn-icon">\n                  <use xlink:href="#remove"></use>\n                </svg>\n              </button>\n              <button type="button" class="btn-del" data-popup="#popup">\n                <svg class="btn-icon">\n                  <use xlink:href="#del"></use>\n                </svg>\n              </button>\n            </div>\n          </td>\n        </tr>\n    `);
        }));
    }
    if (customersRows) addCustomers(customers_namespaceObject, customersRows);
    let sortCustomers = customers_namespaceObject.slice();
    function sort(value) {
        customersRows.innerHTML = "";
        switch (value) {
          case "userName":
            sortCustomers.sort((function(a, b) {
                return a.name.localeCompare(b.name);
            }));
            addCustomers(sortCustomers, customersRows);
            break;
          ...
          default:
            addCustomers(customers_namespaceObject, customersRows);
        }
    }
    const btnSortName = document.querySelector(".btn-sort--name");
    if (btnSortName) btnSortName.addEventListener("click", (ev => {
        sort("userName");
    }));
    const btnSortDate = document.querySelector(".btn-sort--date");
    if (btnSortDate) btnSortDate.addEventListener("click", (ev => {
        sort("date");
    }));
    const {fromEvent} = rxjs;
    const {map, filter, distinctUntilChanged, debounceTime, switchMap, catchError} = rxjs.operators;
    const searchInput = document.querySelector(".oc-search-input");
    const searchResult = document.querySelector("#oc-search-result");
    const searchUserStream$ = fromEvent(searchInput, "input").pipe(map((ev => ev.target.value.trim())), filter((value => value.length >= 3)), debounceTime(500), distinctUntilChanged(), switchMap((searchValue => {
        let arr = [ ...customers_namespaceObject ].filter((customer => customer.name.toLowerCase().includes(searchValue.toLowerCase())));
        if (arr.length !== 0) {
            customersRows.innerHTML = "";
            addCustomers(arr, customersRows);
        } else {
            customersRows.innerHTML = "";
            addCustomers(customers_namespaceObject, customersRows);
    ...
    searchUserStream$.subscribe();
    new MouseFollower({
        stateDetection: {
            "-pointer": "a, button",
            "-hidden": "input",
            "-exclusion": ".logo__block"
        }
    });
    const btnsDelUser = document.querySelectorAll(".btn-del");
    if (btnsDelUser) btnsDelUser.forEach((btn => {
        btn.addEventListener("click", (ev => {
            const user = btn.closest(".user");
            user.classList.add("to-delete");
        }));
    }));
    function removeClass() {
        customersRows.querySelectorAll(".user").forEach((row => {
            row.classList.remove("to-delete");
        }));
    }
    Array.from(customersRows.querySelectorAll(".user"));
    function delUser() {
    ...
    class Popup {
        ...
        eventsPopup() {
            document.addEventListener("click", function(e) {
                ...
                const btnPopupOk = e.target.closest(".popup__ok");
                if (btnPopupOk) {
                    delUser();
                    this.close();
                }
